...
namespace ve {
    ...
    VmaAllocationInfo VeDevice::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VmaAllocationCreateFlags creationFlags, VkBuffer& buffer, VmaAllocation& bufferMemory) {
        ...
        allocInfo.flags = creationFlags;
        VmaAllocationInfo allocationInfo{};
        vmaCreateBuffer(allocator, &bufferInfo, &allocInfo, &buffer, &bufferMemory, &allocationInfo);
        return allocationInfo;

    }
    void VeDevice::destroyBuffer(VkBuffer& buffer, VmaAllocation& bufferMemory) {
        vmaDestroyBuffer(allocator, buffer, bufferMemory);
    }

    void VeDevice::destroyImage(VkImage& image, VmaAllocation& imageAllocation) {
    ...
    void VeDevice::fillBuffer(VmaAllocation& bufferMemory, VkDeviceSize offset, VkDeviceSize size, const void* _src) {

        vmaCopyMemoryToAllocation(allocator, _src, bufferMemory, offset, size);

    }
    bool VeDevice::hasStencilComponent(VkFormat format) {
        return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
    }
    void VeDevice::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
        VkCommandBuffer commandBuffer = beginSingleTimeCommands();
    ...
    VkPhysicalDeviceLimits VeDevice::getDeviceLimits() {
    ...
    void VeDevice::flushAllocation(VmaAllocation &bufferMemory,VkDeviceSize size) {
        if (vmaFlushAllocation(allocator, bufferMemory, 0, size)!=VK_SUCCESS) {
